...
(1,'123@gmail.com','Mike',null),
(2,'456@gmail.com',null,'Highway')
...
Select Coalesce(Firstname,Lastname)as no_null_value from employees
...
select * from Products
where 
Price < (select avg(Price) * 0.1 from Products)
...
select * from Orders
where
orderdate between (select dateadd(day,-180,max(orderdate)) from orders) and (select max(orderdate) from orders)
SELECT GETDATE();
